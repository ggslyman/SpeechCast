...
* 14/03/01 2.0.0 Rev.1 β5
  - 代替字幕追加の影響で、レス読み後即字幕が消えていた問題が発生していたため、レス読み上げ後の表示時間オプション追加

* 14/03/01 2.0.0 Rev.1 β4
  - 代替字幕関係の処理が正常動作していなかった問題を修正

* 14/03/01 2.0.0 Rev.1 β3
  - beta2で自動次スレオープンが動作していなかった問題を修正
  - また、Webアクセス別スレッドが正常動作していたようなので、Webアクセス周りと読み上げ周りのタスクを整理

* 14/03/01 2.0.0 Rev.1 β2
